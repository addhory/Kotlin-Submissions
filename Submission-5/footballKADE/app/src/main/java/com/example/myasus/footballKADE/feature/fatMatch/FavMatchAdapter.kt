...
class FavoriteViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
    ...
    fun bind (matchFavorite: MatchFavorite, listener: (MatchFavorite) -> Unit){
        if (matchFavorite.intAwayScore==null){
            ...
            eventTime.setTextColor(eventDate.context.resources.getColor(R.color.colorAccent))
        }
        val timeEvent = toGMTFormat(matchFavorite.dateEvent, matchFavorite.dateTime)
        eventTime.text= SimpleDateFormat("HH:mm").format(timeEvent)
        Log.d("dateTime", "DateTime adalah " + timeEvent)
